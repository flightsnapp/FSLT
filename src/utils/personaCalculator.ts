...
// Export progress teasers from JSON (for use in Quiz.tsx or similar)
export const PROGRESS_TEASERS = {
  "3": [
    {
      "condition": "scores.Openness >= 7",
      "message": "You’re leaning adventurous—let’s see if you’re a planner or a free spirit!"
    },
    {
      "condition": "scores.Openness >= 4",
      "message": "You’re balanced—open to some new things but not too wild. Let’s keep going!"
    },
    {
      "condition": "true",
      "message": "You’re a comfort-seeker—wonder if you’re a planner or a free spirit!"
    }
  ],
  "6": [
    {
      "condition": "scores.Extraversion >= 7",
      "message": "You’re quite the social butterfly—let’s see what else defines you!"
    },
    {
      "condition": "true",
      "message": "You enjoy your own company—let’s see what else defines you!"
    }
  ],
  "9": [
    {
      "condition": "scores.Conscientiousness >= 7",
      "message": "You’re a planner—almost there!"
    },
    {
      "condition": "true",
      "message": "You go with the flow—let’s wrap this up!"
    }
  ],
  "default": {
    "message": "Keep going to uncover your unique style!"
  }
};
...
export const calculateTraitScores = (questionScores: number[]): { raw: Scores; centered: Scores; strength: Scores } => {
  ...
  return { raw, centered: raw, strength: raw }; // Use raw scores directly for personaFit
};
...
export const getPersona = (traitScores: Scores, followUpTags: string[] = []): Persona => {
  ...
  // Call personaFit with raw 0..100 scores
...
export async function encryptQuizResult(data: { persona: Persona; scores: number[]; date: Date }): Promise<string> {
  try {
    if (!data || !data.persona || !Array.isArray(data.scores) || !(data.date instanceof Date)) {
    ...
    console.error('encryptQuizResult: Encryption failed', error instanceof Error ? error.message : error);
...
export async function decryptQuizResult(encryptedData: string): Promise<{ persona: Persona; scores: number[]; date: Date } | null> {
  try {
    if (!encryptedData || typeof encryptedData !== 'string') {
      console.warn('decryptQuizResult: Invalid or empty encrypted data');
      return null;
    }
    ...
    try {
      ...
      console.error('decryptQuizResult: Invalid base64 encoding', e instanceof Error ? e.message : e);
      return null;
    }
    ...
    if (bytes.length < 12) {
      console.error('decryptQuizResult: Data too short for IV (length:', bytes.length, ')');
      return null;
    }
    ...
    if (ct.length === 0) {
      console.error('decryptQuizResult: No ciphertext after IV');
      return null;
    }
    ...
    let result: { persona: Persona; scores: number[]; date: string };
    try {
      result = JSON.parse(decoded);
    } catch (e: unknown) {
      console.error('decryptQuizResult: JSON parse failed', e instanceof Error ? e.message : e);
      return null;
    }
    ...
    if (
      ...
      !result.date ||
      !PERSONAS.some(p => p.name === result.persona.name)
    ) {
      console.error('decryptQuizResult: Invalid result structure', result);
      return null;
    }

    return { ...result, date: new Date(result.date) };
  } catch (error: unknown) {
    console.error('decryptQuizResult: Decryption failed', error instanceof Error ? error.message : error);
    return null;
  }
}
